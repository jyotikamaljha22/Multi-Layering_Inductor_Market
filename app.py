    ...
    st.markdown(
        ...
        <style>
            ...
            .login-brand {{
                ...
                position: relative;
                overflow: hidden;
                box-shadow: 0 22px 46px rgba(91,15,46,0.22);
            }}
...
def chapter_visuals(title: str) -> None:
    if title == "3. Total Addressable Market (TAM) & Growth Forecast (2025–2035)":
        col1, col2 = st.columns([1.2, 1])
        with col1:
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=market_df["Year"], y=market_df["Revenue"], mode="lines+markers+text",
                text=[f"${x:,.0f}M" for x in market_df["Revenue"]],
                textposition="top center",
                line=dict(color=BURGUNDY, width=4), marker=dict(size=10, color=BURGUNDY)
            ))
            fig.update_yaxes(title_text="Revenue ($Mn)")
            style_plot(fig, "TAM expansion through 2035", 340)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(go.Bar(x=tam_df["Year"], y=tam_df["Device Anchors"], name="Device Anchors (Mn)", marker_color="#D7B2C0"), secondary_y=False)
            fig.add_trace(go.Scatter(x=tam_df["Year"], y=tam_df["Components per Device"], name="Components per Device", mode="lines+markers", line=dict(color=BURGUNDY, width=4), marker=dict(size=9, color=BURGUNDY)), secondary_y=True)
            fig.update_yaxes(title_text="Device Anchors (Mn)", secondary_y=False)
            fig.update_yaxes(title_text="Components / Device", secondary_y=True)
            style_plot(fig, "Density dynamics", 340)
            st.plotly_chart(fig, use_container_width=True)

    elif title == "4. Market Segmentation & Value Pools":
        col1, col2 = st.columns([1.15, 1])
        with col1:
            melt = segment_df.melt(id_vars="Segment", var_name="Year", value_name="Revenue")
            fig = px.bar(melt, x="Year", y="Revenue", color="Segment", barmode="group", color_discrete_sequence=[BURGUNDY, "#B07A92", "#E7C7D5"])
            style_plot(fig, "Segment evolution", 340)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = go.Figure(data=[go.Pie(labels=["RF", "Compact Ferrite / NFC", "Automotive"], values=[54.0, 31.7, 14.1], hole=0.62, marker=dict(colors=[BURGUNDY, "#B07A92", "#E7C7D5"]))])
            fig.update_traces(textinfo="label+percent")
            style_plot(fig, "2035 revenue mix", 340)
            st.plotly_chart(fig, use_container_width=True)

    elif title == "5. Demand Engine: Device-Level Volume Modelling":
        melt = device_df.melt(id_vars="Device", value_vars=["2025", "2035"], var_name="Year", value_name="Volume")
        fig = px.bar(melt, x="Device", y="Volume", color="Year", barmode="group", color_discrete_sequence=[BURGUNDY, "#D7B2C0"])
        fig.update_layout(xaxis_tickangle=-18)
        style_plot(fig, "Device-level shipment comparison", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "7. Pricing Dynamics & Value Realization":
        melt = pricing_df.melt(id_vars="Segment", var_name="Year", value_name="ASP")
        fig = px.line(melt, x="Year", y="ASP", color="Segment", markers=True, color_discrete_sequence=[BURGUNDY, "#A4456F", "#D7B2C0", "#9F6B83"])
        style_plot(fig, "ASP erosion by segment", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "8. Regional Market Analysis: North America":
        melt = north_america_df.melt(id_vars="Region", var_name="Year", value_name="Revenue")
        fig = px.bar(melt, x="Region", y="Revenue", color="Year", barmode="group", color_discrete_sequence=[BURGUNDY, "#D7B2C0", "#EED7E1"])
        style_plot(fig, "North America revenue profile", 340)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "9. Regional Market Analysis: Europe":
        melt = europe_df.melt(id_vars="Region", var_name="Year", value_name="Revenue")
        fig = px.bar(melt, x="Region", y="Revenue", color="Year", barmode="group", color_discrete_sequence=[BURGUNDY, "#D7B2C0", "#EED7E1"])
        style_plot(fig, "Europe revenue profile", 340)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "10. Regional Market Analysis: Asia Pacific (Ex-Japan) & Japan":
        melt = apac_df.melt(id_vars="Region", var_name="Year", value_name="Revenue")
        fig = px.bar(melt, x="Region", y="Revenue", color="Year", barmode="group", color_discrete_sequence=[BURGUNDY, "#D7B2C0", "#EED7E1"])
        style_plot(fig, "Asia Pacific + Japan revenue profile", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == CH11_TITLE:
        col1, col2 = st.columns([1.15, 1])
        with col1:
            melt = competitive_df.melt(id_vars="Competitor", var_name="Year", value_name="Share")
            fig = px.bar(melt, x="Competitor", y="Share", color="Year", barmode="group", color_discrete_sequence=[BURGUNDY, "#D7B2C0"])
            style_plot(fig, "Market share (anonymized)", 340)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            comp = competitive_df.copy()
            comp["Change"] = comp["2035"] - comp["2025"]
            fig = px.bar(comp, x="Change", y="Competitor", orientation="h", color="Change", color_continuous_scale=["#D7B2C0", BURGUNDY], range_color=[comp["Change"].min(), comp["Change"].max()])
            style_plot(fig, "Share change vs. 2025", 340)
            st.plotly_chart(fig, use_container_width=True)
        st.markdown('<div class="chapter-note">For confidentiality, competitor identities are intentionally masked as Player 1, Player 2, and so on in this chapter and its supporting visuals.</div>', unsafe_allow_html=True)

    elif title == "15. Product Benchmarking & Miniaturization (0402 to 01005)":
        fig = px.scatter(
            miniaturization_df, x="Barrier Score", y="Strategic Value Score", size=[20, 28, 36, 44],
            text="Package", color="Package", color_discrete_sequence=[BURGUNDY, "#A4456F", "#C58CA6", "#E7C7D5"]
        )
        fig.update_traces(textposition="top center")
        fig.update_xaxes(title_text="Manufacturing barrier")
        fig.update_yaxes(title_text="Strategic value")
        style_plot(fig, "Miniaturization frontier", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "16. Capacity & Supply-Demand Balance":
        fig = px.scatter(
            capacity_df, x="Risk", y="Reward", size="Barrier", color="Node",
            color_discrete_sequence=[BURGUNDY, "#A4456F", "#D7B2C0"], text="Node"
        )
        fig.update_traces(textposition="top center")
        fig.update_xaxes(title_text="Risk profile")
        fig.update_yaxes(title_text="Strategic reward")
        style_plot(fig, "Capacity node positioning", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "18. Strategic Scenarios (Base, Upside, Downside) & Sensitivity":
        fig = go.Figure()
        for name, color in [
            ("Base Case Reality", BURGUNDY),
            ("Optimized Upside", "#A4456F"),
            ...
            fig.add_trace(go.Scatter(
                x=scenario_df["Year"], y=scenario_df[name], mode="lines+markers+text",
                text=[f"${x:,.0f}M" for x in scenario_df[name]],
                textposition="top center" if name == "Optimized Upside" else "bottom center",
                name=name, line=dict(width=4, color=color), marker=dict(size=9, color=color)
            ))
        fig.update_yaxes(title_text="Revenue ($Mn)")
        style_plot(fig, "Scenario envelope through 2035", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "19. TDK Strategy: Opportunity Mapping & Win/Loss Matrix":
        fig = px.bar(
            win_loss_df, x="Outcome Score", y="Vector", orientation="h",
            text="Label", color="Outcome Score",
            color_continuous_scale=["#E8C7D5", "#C07A97", BURGUNDY],
            range_color=[1, 5]
        )
        fig.update_traces(textposition="outside")
        fig.update_xaxes(title_text="Strategic attractiveness")
        fig.update_yaxes(title_text="")
        style_plot(fig, "TDK opportunity map", 360)
        st.plotly_chart(fig, use_container_width=True)

    elif title == "20. Strategic Recommendations & Investment Roadmap":
        fig = px.bar(
            roadmap_df.sort_values("Priority Score"), x="Priority Score", y="Priority",
            orientation="h", text="Time Horizon", color="Priority Score",
            color_continuous_scale=["#E8C7D5", "#C07A97", BURGUNDY],
            range_color=[75, 100]
        )
        fig.update_xaxes(title_text="Priority score")
        fig.update_yaxes(title_text="")
        style_plot(fig, "Investment roadmap priorities", 360)
        st.plotly_chart(fig, use_container_width=True)
...
def render_prev_next(options: list[str], current: str) -> None:
    ...
    with mid:
        st.markdown(
            f'<div class="nav-tip" style="text-align:center;">Section {idx} of {len(options)-1} · Use the sidebar to jump across the full 20-chapter report.</div>',
            unsafe_allow_html=True,
        )
    with right:
...
def render_chapter_page(title: str, nav_options: list[str]) -> None:
    st.markdown(
        ...
        <div class="report-shell">
            <div class="report-banner">
                <div class="report-kicker">Full chapter view</div>
    ...
    chapter_visuals(title)
    st.markdown(CHAPTERS[title], unsafe_allow_html=True)
    st.markdown(
        """
        <div class="footnote-note">
            This chapter view preserves the full source narrative and table structure provided in the original HTML, with additional dashboard visuals layered above for executive readability.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_login() -> None:
    ...
    st.markdown('<div class="login-screen"><div class="login-wrap">', unsafe_allow_html=True)
    ...
    with left:
        st.markdown(
            ...
            <div class="login-brand">
                <div class="login-kicker">Strategic Market Research</div>
                <div class="login-title">Boardroom-grade market intelligence — presented as a dynamic executive dashboard.</div>
                <div class="login-copy">
                    This secure preview translates the full multilayer inductor report into a polished Streamlit experience with visual storytelling, premium interface treatment, and intact chapter fidelity. Access is intentionally simple for client presentation use.
                </div>
                <div class="login-chip-row">
                    <div class="login-chip">20 full chapters retained</div>
                    <div class="login-chip">Premium burgundy presentation layer</div>
                    <div class="login-chip">Confidential competitive masking</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with right:
        ...
        st.markdown('<div class="login-form-title">SMR Boardroom Access</div>', unsafe_allow_html=True)
        st.markdown('<div class="login-form-sub">Enter your name and the shared preview password to open the dashboard.</div>', unsafe_allow_html=True)
        with st.form("login_form", clear_on_submit=False):
            ...
            if submitted:
                ...
                elif password == PRIMARY_PASSWORD:
